...
### Problem 1: Put all the multiples of 15 from 1 to 1000 in a vector (save it as a variable named multiples_15) (3 pts)
```{r}
#FILL IN HERE
```
...
## Problem 2: Create a 20x10 matrix where each row is a random integer array with a minimum of 1, max of 9 (inclusive). Then for each row, sum all the values and add the sum to a vector of values called 'random_sums' (4 points)
...
## Problem 3: Find the numbers that are in both 'multiples_15' and 'random_sums' and put them in an array called both (2 pts)
...
** Size of an array ** 
To find the size of arrays/vectors in R, use the length() function
```{r}
...
** Sorting an array ** 
To sort an array/vector use the sort() function
```{r}
arr1 = c(8,4,9,2,3,1,10,12)
print(sort(arr1))

arr2 = c('bob', 'david', 'chen', 'joseph')
print(sort(arr2))

#Sorting in reverse order
print(sort(arr1, decreasing = T))
```


## Problem 4: Print the size of the array you found in Problem 3 above. Then, print the sorted array. (1 pt.)
```{r}
## Your code here (should only be 2 lines)
```
